...
export const VotingCandidateCard = ({
  ...
  const turnoutPercentage =
    maxVotes > 0
      ? Math.min(Math.round((candidate.votes / maxVotes) * 100), 100)
      : 0;
      ...
      <Link href={`/candidates/${candidate.balota_number}`}>
          ...
          >
            <div className="absolute inset-0 flex flex-col">
              ...
              <div className="mt-auto p-3 pt-0 bg-gradient-to-t from-black/70 to-transparent">
                <p className="uppercase leading-tight m-0 text-2xl font-bebas text-wrap max-w-40">
                  {candidate.name}
                </p>
              ...
              <Image
                src={
                  getCandidateImage(candidate.name) ||
                  "/public/candidates/DUTERTE-PLACEHOLDER.png"
                }
                alt={candidate.name}
                width={192}
                height={192}
        ...
        <div className="hidden sm:block">
          ...
          >
            <>
              <div
                className={`p-4 ${isPhotoLeft && "flex flex-col items-end"}`}
              >
                {!isLongCard && (
                  <>
                    <p
                ...
                {!isLongCard && (
                  <div
                    className={`w-full mt-2 ${isPhotoLeft && "flex flex-col items-end"}`}
                    ref={desktopBarContainerRef}
                  >
