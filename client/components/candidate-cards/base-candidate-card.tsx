  ...
  return (
    <motion.div variants={fadeUp} transition={{ delay: 0.1 + index * 0.1 }}>
      <Link href={`/candidates/${candidate.balota_number}`}>
        <ImageDiv bgImage={PARTYLIST_TO_IMAGE[candidate.party_list]} className="flex md:gap-4 px-4 items-center h-40">
          <>
            <div className="w-40">
              <Image
            ...
            <div className="flex md:flex-row flex-col w-full items-end justify-between">
              <div className="space-y-1">
                <h1 className="text-base md:text-3xl font-bold">{candidate.name}</h1>
                <p className="italic text-xs md:text-sm">For {candidate.position}</p>
              </div>
