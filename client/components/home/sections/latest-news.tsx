...
export default function LatestNewsSection() {
  ...
  return (
    <motion.section className="border rounded-lg p-4 relative overflow-hidden" variants={fadeUp}>
      <div
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {data.length > 0 ? (
              ...
              <div className="text-center py-6 text-gray-400">No news available at the moment</div>
            )}
          </div>
        )}
      </div>
  ...
  const parseMessage = (message: string) => {
    ...
    if (breakIndex === -1) {
      ...
      if (capitalWordMatch && capitalWordMatch.index !== undefined) {
        ...
        breakIndex = spaceAfter50 > 0 ? spaceAfter50 : Math.min(50, message.length)
      }
    }
