  ...
  return (
    <div className="py-2">
        ...
        {stages.map((stage, index) => (
          <div key={`date-${index}`} className="text-center w-1/4">
            <span className={`text-sm ${dateToday > stage.date ? 'text-[#FF6969]' : 'text-neutral-400'}`}>
              {stage.date.toLocaleDateString('en-US', { month: 'numeric', day: 'numeric' })}
            </span>
          </div>
        ))}
      </div>
      ...
      <div className="relative flex items-center justify-between px-2">
        <div className="absolute right-8 left-12 md:left-20 md:right-14 h-0.5 bg-[#FF6969]/30 top-1/2 transform -translate-3 z-0"></div>
