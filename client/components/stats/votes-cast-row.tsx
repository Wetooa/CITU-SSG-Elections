...
export const VotesCastRow = ({ position, candidates, index }: VotesCastRowProps) => {
  ...
  const totalVotesForPosition = candidates.reduce((sum, candidate) => sum + candidate.votes, 0)
  ...
  partyLists.forEach((party) => {
    partyTotalVotes[party] = candidatesByParty[party].reduce((sum, candidate) => sum + candidate.votes, 0)
  })

  const winningParty = partyLists.reduce((a, b) => (partyTotalVotes[a] > partyTotalVotes[b] ? a : b), partyLists[0])
      ...
      {partyLists.length === 2 && candidates.length > 1 && (
        <div className="flex flex-col sm:flex-row items-stretch gap-3 sm:gap-4">
          ...
          <div className="flex sm:flex-col flex-row gap-2 items-center justify-center my-1 sm:my-0">
            ...
            <span className="text-base sm:text-lg font-semibold text-center px-2 font-sans">vs.</span>
            <div className="border-t sm:border-l sm:h-6 w-6 sm:w-auto" />
          </div>

          {/* Second party */}
          <div className="w-full sm:w-1/2 space-y-3">
            {candidatesByParty[partyLists[1]].map((candidate, candidateIndex) => (
              <VotingCandidateCard
