...
export default function CandidatesPage() {
  ...
  useEffect(() => {
    ...
    const filteredCandidates = candidatesData.filter((candidate) => {
      const matchesName = selectedName === ALL || candidate.name.toLowerCase().includes(selectedName.toLowerCase())

      const matchesPartyList = selectedPartyList === ALL || candidate.party_list === selectedPartyList
  ...
  }, [candidatesData, isLoading, selectedName, selectedPartyList, selectedPosition])

  return (
    <motion.div className="p-4 space-y-6 w-full" initial="initial" animate="animate" exit="exit">
      <motion.div
        className="flex md:flex-row flex-col w-full items-center justify-between"
        variants={fadeDown}
        transition={{ delay: 0.1 }}
      >
        <motion.h1 className="text-3xl font-bold text-white">Candidates</motion.h1>
      ...
      <motion.div className="border p-4 rounded-lg space-y-2" initial="initial" animate="animate" exit="exit">
        {candidates.length === 0 ? (
          ...
          candidates.map((candidate, index) => <BaseCandidateCard key={index} index={index} candidate={candidate} />)
        )}
      </motion.div>
    </motion.div>
  )
